...
const identifyAccount = async (accountId) => {
  ...
  // For a guide on the data type for different fields, visit https://docs.hyperengage.io/getting-started/understanding-our-data-model 
    ...
    const payload = {
      ...
      account_id: accountId,// Unique account_id in DB (Mandatory)
    ...
    try {
        ...
        console.log('Account identified:', response.data);
        return response.data;
    } catch (error) {
        console.error('Error identifying account:', error.message);
        throw new Error('Error identifying account.');
    }
};
